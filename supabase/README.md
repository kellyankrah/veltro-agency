...
2. `migrations/0002_tedx_storage.sql` — the `tedx-resumes` (private) and
   `tedx-headshots` (public-read) storage buckets used by the speaker
   application's file uploads.
...
`status` columns each table carries for exactly this purpose), so an admin
UI can be built directly against those types without touching this schema.
