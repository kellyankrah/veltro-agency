...
- `/speakers` → `SpeakersIndexPage` — currently redirects to `/#speakers`
  on the landing page. Swap this for a real standalone index (reusing the
  same card grid as the landing section) once there's a reason to give
  speakers their own hub separate from the homepage.
...
1. Add an entry to `SPEAKERS` in `src/data/speakers.ts` — `id` doubles as
   the URL slug.
